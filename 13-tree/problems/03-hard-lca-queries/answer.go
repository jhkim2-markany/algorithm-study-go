package main

import (
	"bufio"
	"fmt"
	"os"
)

const MAXLOG = 17

var (
	adj   [][]int
	depth []int
	up    [MAXLOG][]int
)

// buildLCA는 트리의 깊이와 희소 테이블을 구축한다.
//
// [매개변수]
//   - adj: 인접 리스트 (1-indexed)
//   - n: 노드 수
//   - root: 루트 노드 번호
//
// [알고리즘 힌트]
//
//	DFS로 각 노드의 깊이와 직접 부모(up[0])를 구한 뒤,
//	희소 테이블을 구축한다: up[k][v] = v의 2^k번째 조상.
//	up[k][v] = up[k-1][up[k-1][v]] 점화식을 사용한다.
func buildLCA(adj [][]int, n, root int) {
	var dfs func(cur, par, d int)
	dfs = func(cur, par, d int) {
		depth[cur] = d
		up[0][cur] = par
		for _, next := range adj[cur] {
			if next != par {
				dfs(next, cur, d+1)
			}
		}
	}
	dfs(root, 0, 0)

	for k := 1; k < MAXLOG; k++ {
		for v := 1; v <= n; v++ {
			up[k][v] = up[k-1][up[k-1][v]]
		}
	}
}

// queryLCA는 두 노드의 최소 공통 조상을 반환한다.
//
// [매개변수]
//   - u: 첫 번째 노드 번호
//   - v: 두 번째 노드 번호
//
// [반환값]
//   - int: 두 노드의 최소 공통 조상 노드 번호
//
// [알고리즘 힌트]
//
//	이진 리프팅(Binary Lifting)을 사용한다.
//	먼저 두 노드의 깊이를 맞춘 뒤,
//	두 노드를 동시에 올려서 LCA 직전까지 이동하고,
//	한 칸 더 올리면 LCA가 된다.
func queryLCA(u, v int) int {
	if depth[u] < depth[v] {
		u, v = v, u
	}

	diff := depth[u] - depth[v]
	for k := 0; k < MAXLOG; k++ {
		if (diff>>k)&1 == 1 {
			u = up[k][u]
		}
	}

	if u == v {
		return u
	}

	for k := MAXLOG - 1; k >= 0; k-- {
		if up[k][u] != up[k][v] {
			u = up[k][u]
			v = up[k][v]
		}
	}

	return up[0][u]
}

func main() {
	reader := bufio.NewReader(os.Stdin)
	writer := bufio.NewWriter(os.Stdout)
	defer writer.Flush()

	// 노드 수 입력
	var n int
	fmt.Fscan(reader, &n)

	// 인접 리스트 초기화
	adj = make([][]int, n+1)
	depth = make([]int, n+1)
	for k := 0; k < MAXLOG; k++ {
		up[k] = make([]int, n+1)
	}
	for i := 0; i <= n; i++ {
		adj[i] = []int{}
	}

	// 간선 입력
	for i := 0; i < n-1; i++ {
		var u, v int
		fmt.Fscan(reader, &u, &v)
		adj[u] = append(adj[u], v)
		adj[v] = append(adj[v], u)
	}

	// LCA 전처리
	buildLCA(adj, n, 1)

	// 쿼리 처리
	var m int
	fmt.Fscan(reader, &m)
	for i := 0; i < m; i++ {
		var a, b int
		fmt.Fscan(reader, &a, &b)
		fmt.Fprintln(writer, queryLCA(a, b))
	}
}
